...
contract('SplitPayment', (accounts) => {
  ...
  it("Should split payment", async () => {
    ...
    const initialBalances = await Promise.all(recipients
      .map(recipient => {
    ...
    const finalBalances = await Promise.all(recipients
      .map(recipient => {
        return web3.eth.getBalance(recipient);
      })
    );
  ...
  it("Should NOT split playment if array length mismatch", async () => {
    const recipients = [accounts[1],accounts[2],accounts[3]];
    const amounts = [40, 20];
